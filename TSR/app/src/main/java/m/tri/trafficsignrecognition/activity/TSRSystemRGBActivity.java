...
import m.tri.trafficsignrecognition.activity.ui.FaceOverlayView;
...
import m.tri.trafficsignrecognition.utils.CameraErrorCallback;
import m.tri.trafficsignrecognition.utils.ImageUtils;
import m.tri.trafficsignrecognition.utils.Util;
...
public final class TSRSystemRGBActivity extends AppCompatActivity implements SurfaceHolder.Callback, Camera.PreviewCallback {

    // Number of Cameras in device.
    private int numberOfCameras;

    public static final String TAG = TSRSystemRGBActivity.class.getSimpleName();
    ...
    long startnow;
    long endnow;
    boolean savePreviewToBmp = false;
...
//                recreate();
    ...
    private void setDisplayOrientation() {
        ...
        mDisplayRotation = Util.getDisplayRotation(TSRSystemRGBActivity.this);
    ...
    private void configureCamera(int width, int height) {
        ...
        //for (int i: parameters.getSupportedPreviewFormats()) { Log.e(TAG, "preview format supported are = "+i);}
...
////                mGLRgbBuffer = IntBuffer.allocate(previewWidth * previewHeight);
////            }
////
////            GPUImageNativeLibrary.YUVtoRBGA(_data, previewWidth, previewHeight, mGLRgbBuffer.array());
////
////            System.out.println("Byte[0] = " + mGLRgbBuffer.get(0));
////            System.out.println("Byte[1] = " + mGLRgbBuffer.get(1));
////            System.out.println("Byte[2] = " + mGLRgbBuffer.get(2));
//
//            startnow = android.os.SystemClock.uptimeMillis();
//
//            int[] mGLRgbBuffer = new int[previewWidth*previewHeight];
//
//            GPUImageNativeLibrary.YUVtoARBG(_data, previewWidth, previewHeight, mGLRgbBuffer);
//
//            endnow = android.os.SystemClock.uptimeMillis();
//            Log.d("YUV TO RGBA", "Execution time: " + (endnow - startnow) + " ms");
//
//            System.out.println("Byte[0] = " + (mGLRgbBuffer[0] & 0xFF));
//            System.out.println("Byte[1] = " +((mGLRgbBuffer[0] >> 8) & 0xFF));
//            System.out.println("Byte[2] = " + ((mGLRgbBuffer[0] >> 16) & 0xFF));
//
//            isBusy = false;
//
//        }
...
//            YuvImage yuv = new YuvImage(_data, ImageFormat.NV21,
...
//
//            endnow = android.os.SystemClock.uptimeMillis();
//            Log.d("CAMARA TO RGB565", "Execution time: " + (endnow - startnow) + " ms");
//
//        }
        ...
        if(savePreviewToBmp)
        {
            ...
            if(config.orientation == Configuration.ORIENTATION_PORTRAIT)
            {
                ...
                Bitmap binaryImg = Bitmap.createBitmap(w,h,Bitmap.Config.ARGB_8888);
                ...
                for(int j = h-1; j >= 0; j--)
                {
                    ...
                    for(int i = w-1 ; i >=0 ; i--, x++)
                    {
                        if( (j > 635) && (i > 400))
                        {
                            binaryImg.setPixel(x,j,(0xFF << 24) + (0xFF << 16) + (0xFF << 8) + (0xFF));
                        }
                        else
                        {
                            binaryImg.setPixel(x, j, (0xFF << 24) + (0x00 << 16) + (0x00 << 8) + (0x00));
                        }


                        //binaryImg.setPixel(x,j,mGLRgbBuffer[j+i*previewWidth]);
                    }
                }

                // Save the binary image before applying erosion and dilation
                File dest = new File("storage/sdcard1/TCC-TSR-2017/SURF implementation/PictureYUV_TO_RGB.png");

                TSRSystem tsrSystem = new TSRSystem();

                tsrSystem.saveBmp(dest, binaryImg);
            ...
            // Create a new reference to the binary image.
            Bitmap binaryImg = Bitmap.createBitmap(previewWidth,previewHeight,Bitmap.Config.ARGB_8888);

            for(int j = 0; j < previewHeight; j++)
            {
                for(int i = 0; i < previewWidth; i++)
                {
                    binaryImg.setPixel(i,j,mGLRgbBuffer[i+j*previewWidth]);
                }
            }
...
//            Log.i("FaceDetectThread", "running");
    ...
    private void resetData() {
        if (imagePreviewAdapter == null) {
            ...
            imagePreviewAdapter = new ImagePreviewAdapter(TSRSystemRGBActivity.this, facesBitmap, new ImagePreviewAdapter.ViewHolder.OnItemClickListener() {
                @Override
